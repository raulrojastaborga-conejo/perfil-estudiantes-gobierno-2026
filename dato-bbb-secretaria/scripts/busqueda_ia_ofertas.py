...
HOME_PATTERNS = ["/", "/tienda", "/search", "/listado", "/ofertas", "/categoria"]
...
def get(url: str, accept: str = "text/html") -> str | None:
    req = Request(url, headers={"User-Agent": "Mozilla/5.0 DatoBBBSearch/1.0", "Accept": accept, "Accept-Language": "es-CL,es;q=0.9"})
    try:
...
def is_product_url(url: str) -> bool:
    ...
    if path in HOME_PATTERNS:
        return False
    if any(x in path for x in ["/cart", "/login", "/ayuda", "/help", "/seller", "/brand"]):
        return False
    return len(url) > 35
...
def search_brave(query: str) -> list[str]:
    ...
    if not key:
        return []
    url = "https://api.search.brave.com/res/v1/web/search?" + urlencode({"q": query, "country": "CL", "search_lang": "es", "count": 10})
...
def search_tavily(query: str) -> list[str]:
    ...
    if not key:
        return []
    body = json.dumps({"api_key": key, "query": query, "search_depth": "basic", "max_results": 10, "include_answer": False}).encode("utf-8")
...
def search_web(query: str) -> list[str]:
    full_query = query + " precio oferta Chile site:mercadolibre.cl OR site:falabella.com OR site:ripley.cl OR site:paris.cl OR site:dbs.cl OR site:maicao.cl OR site:preunic.cl"
    urls = []
...
def extract_price(page: str) -> int:
    candidates = []
    for pat in [r'product:price:amount["\'][^>]+content=["\']([0-9\.]+)', r'"price"\s*:\s*"?([0-9\.]{4,})', r'\$\s*([0-9\.]{4,})']:
...
def score(price: int, category: str, title: str) -> dict[str, int]:
    ...
    bonito = 4 if any(x in tl for x in ["elegante", "blazer", "negro", "nude", "rosa", "oficina", "glow", "moderno"]) else 3
...
def main() -> None:
    ...
    collected = []
    status = {"updated_at": now(), "queries": len(config), "urls_found": 0, "products_found": 0, "existing_before": len(existing), "notes": []}
    for item in config:
    ...
    status["notes"].append("Busqueda automatica IA finalizada; se fusiona con datos previos para no perder informacion")
