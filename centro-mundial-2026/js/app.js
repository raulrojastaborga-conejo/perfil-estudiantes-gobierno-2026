...
function oddsHtml(m){
 ...
 const r=(n,v)=>'<div class="odd-row"><span>'+n+'</span><div class="bar"><div class="fill" style="width:'+v+'%"></div></div><b>'+v+'%</b></div>';
 return '<div class="odds"><b>Pronóstico</b>'+r(m.home,o.home_win)+r('Empate',o.draw)+r(m.away,o.away_win)+'<div class="source">Fuente: '+o.source+' · '+o.updated_at+'</div></div>';
}
...
function calcGroup(group){
 ...
 const t={}; gi.teams.forEach(n=>t[n]={team:n,group,pj:0,pg:0,pe:0,pp:0,gf:0,gc:0,dg:0,pts:0});
 matches.filter(m=>m.group===group&&m.gh!==''&&m.ga!=='').forEach(m=>{let h=+m.gh,a=+m.ga;if(isNaN(h)||isNaN(a))return;[m.home,m.away].forEach(n=>{if(!t[n])t[n]={team:n,group,pj:0,pg:0,pe:0,pp:0,gf:0,gc:0,dg:0,pts:0}});t[m.home].pj++;t[m.away].pj++;t[m.home].gf+=h;t[m.home].gc+=a;t[m.away].gf+=a;t[m.away].gc+=h;if(h>a){t[m.home].pg++;t[m.away].pp++;t[m.home].pts+=3}else if(a>h){t[m.away].pg++;t[m.home].pp++;t[m.away].pts+=3}else{t[m.home].pe++;t[m.away].pe++;t[m.home].pts++;t[m.away].pts++}});
 Object.values(t).forEach(x=>x.dg=x.gf-x.gc); return Object.values(t).sort((a,b)=>b.pts-a.pts||b.dg-a.dg||b.gf-a.gf||a.team.localeCompare(b.team));
}
...
function renderPlayers(){const b=$('#players');if(b)b.innerHTML=players.map(p=>'<div class="player"><b>'+p.team+'</b><span>'+p.name+'</span><span class="badge">'+p.position+'</span><span class="badge">'+p.status+'</span></div>').join('')}
